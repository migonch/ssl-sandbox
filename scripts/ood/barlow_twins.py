...
from ssl_sandbox.pretrain import BarlowTwins, BarlowTwinsOODDetection
...
def parse_args():
    ...
    parser.add_argument('--proj_dim', type=int, default=8092)
    parser.add_argument('--lmbd', type=int, default=5e-3)
...
def main(args):
    ...
    callbacks = [
        BarlowTwinsOODDetection(),
        OnlineProbing(model.embed_dim, dm.num_classes),
        LearningRateMonitor(),
        DeviceStatsMonitor(),
        ModelCheckpoint(save_top_k=1, monitor='val/ood_auroc', filename='best', mode='max')
    ]

    logger = TensorBoardLogger(
        save_dir=args.log_dir,
        name=f'pretrain/cifar4vs6/barlow_twins'
    )
