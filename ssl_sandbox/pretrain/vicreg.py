...
class VICRegOODDetection(pl.Callback):
    ...
    def on_validation_batch_end(self, trainer, pl_module: VICReg, outputs, batch, batch_idx, dataloader_idx=0):
        ...
        with eval_mode(self.encoder, enable_dropout=True):
    ...
    def on_validation_epoch_end(self) -> None:
        if any(self.val_ood_auroc.target):
            auroc = self.val_ood_auroc.compute()
        else:
            warnings.warn('No OOD examples in the validation set. Set OOD AUROC to nan.')
            auroc = float('nan')

        self.log('val/ood_auroc', auroc)
